...
var header = "/*!\n" +
  ...
  " * Version: {{ version }}\n" +
  " *\n" +
  " * Copyright 2016 Saito\n" +
  " * Released under the MIT license\n" +
  " * https://github.com/saito\n" +
  " */\n";
...
function buildTask() {
  ...
  var bundled = browserify('./src/saito.js', { standalone: 'Saito' })
    .plugin(collapse)
    .bundle()
    .pipe(source('Saito.bundle.js'))
    ...
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(streamify(concat('Saito.bundle.min.js')))
    .pipe(gulp.dest(outDir));

  var nonBundled = browserify('./src/saito.js', { standalone: 'Saito' })
    .ignore('moment')
    .plugin(collapse)
    .bundle()
    .pipe(source('Saito.js'))
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(gulp.dest(outDir))
    .pipe(streamify(uglify()))
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(streamify(concat('Saito.min.js')))
    .pipe(gulp.dest(outDir));
